...
rootMenuConfig =  {
    ...
    { 
        ...
        subMenus = {"vehicle:park", "vehicle:take", "vehicle:impound"}
    },
    { 
        id = "Auto Tegevused",
        displayName = "Seadistus",
        icon = "#general-car",
    ...
    },
    {
        ...
        enableMenu = function()
            local pData = QBCore.Functions.GetPlayerData()
            return (not pData.metadata["isdead"] and not pData.metadata["inlaststand"] and exports["ks-houses"]:isInHouse())
        end,
        subMenus = {"house:setstash", "house:setoutfit", "house:setlogout", "house:givekeys", "house:decorate", "house:doorlock"}
    },
    {
        id = "HouseOptions",
        displayName = "Kodu",
        ...
        enableMenu = function()
            local pData = QBCore.Functions.GetPlayerData()
            return (not pData.metadata["isdead"] and not pData.metadata["inlaststand"] and not exports["ks-houses"]:isInHouse())
        end,
...
newSubMenus = { -- NOTE basicly, what will be happen after clicking these buttons and icon of them
    ...
    ['police:storegarage'] = {
		...
		enableMenu = function()
			...
			if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() and PlayerData.job.name == 'police' and isInJobGarage and lastJobVehicle ~= nil then  
				...
				if exports["ks-keys"]:HasVehicleKey(plate) and curVeh == lastJobVehicle then
					return true
				end
			end
			return false
    ...
    ['house:setstash'] = {  -- player housing options
        title = "Muuda seifi kohta",
        icon = "#container-storage",
        functionName = "qb-houses:client:setLocation"
    },
    ['house:setoutfit'] = {
        title = "Muuda riiete paika",
        icon = "#blips-clothing",
        functionName = "qb-houses:client:setLocation"
    },
    ['house:setlogout'] = {
        title = "Muuda välja logi",
        icon = "#expressions-sleeping",
        functionName = "qb-houses:client:setLocation"
    },
    ['house:givekeys'] = {
...
AddEventHandler('qb-polyzone:enter', function(name)
    ...
    if name == "garages" then
        inGarage = true
        exports['ks-interaction']:showInteraction('Parking')
    ...
    elseif name == "depots" then
        inDepots = true
        exports['ks-interaction']:showInteraction('Depot')
    elseif name == "Bennys" then
        if InVehicle then
            inBennys = true
            exports['ks-interaction']:showInteraction("Bennys")
        end
    ...
    elseif name == "TunerRepair" then
        if InVehicle then
            ...
            if PlayerJob.name == "tuner" and onDuty then
                exports['ks-interaction']:showInteraction("Repair Station")
            end
        end
    elseif name == "TunerShop" then
        if InVehicle then
            inTunerShop = true
            if PlayerJob.name == "tuner" and onDuty then
                exports['ks-interaction']:showInteraction("6STR Customs")
            end
        end
    ...
    elseif name == "pdmech" then
        if InVehicle then
            inPdMech = true
            exports['ks-interaction']:showInteraction("Vehicle Station")
        end
    ...
    elseif name == "towFinance" then
        if InVehicle then
            inTowFianance = true
            exports['ks-interaction']:showInteraction("Finance Repossession")
        end
...
AddEventHandler('qb-polyzone:exit', function(name)
    if name == "garages" then
        inGarage = false
        exports['ks-interaction']:hideInteraction()
    elseif name == "ganggarages" then
        inGarage = false
        exports['ks-interaction']:hideInteraction()
    elseif name == "jobgarages" then
